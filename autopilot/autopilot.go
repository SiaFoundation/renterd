...
type Bus interface {
	...
	// hostdb
	Host(ctx context.Context, hostKey types.PublicKey) (api.HostInfo, error)
	HostsForScanning(ctx context.Context, opts api.HostsForScanningOptions) ([]hostdb.HostAddress, error)
	RemoveOfflineHosts(ctx context.Context, minRecentScanFailures uint64, maxDowntime time.Duration) (uint64, error)
	SearchHosts(ctx context.Context, opts api.SearchHostOptions) ([]api.HostInfo, error)
...
func countUsableHosts(cfg api.AutopilotConfig, cs api.ConsensusState, fee types.Currency, currentPeriod uint64, rs api.RedundancySettings, gs api.GougingSettings, hosts []api.HostInfo) (usables uint64) {
	gc := worker.NewGougingChecker(gs, cs, fee, currentPeriod, cfg.Contracts.RenewWindow)
	for _, host := range hosts {
		usable, _ := isUsableHost(cfg, rs, gc, host.HostInfo, smallestValidScore, 0)
		if usable {
			usables++
		}
	}
...
func evaluateConfig(cfg api.AutopilotConfig, cs api.ConsensusState, fee types.Currency, currentPeriod uint64, rs api.RedundancySettings, gs api.GougingSettings, hosts []api.HostInfo) (resp api.ConfigEvaluationResponse) {
	gc := worker.NewGougingChecker(gs, cs, fee, currentPeriod, cfg.Contracts.RenewWindow)

	resp.Hosts = uint64(len(hosts))
	for _, host := range hosts {
		usable, usableBreakdown := isUsableHost(cfg, rs, gc, host.HostInfo, 0, 0)
		if usable {
			resp.Usable++
			continue
		}
...
func optimiseGougingSetting(gs *api.GougingSettings, field *types.Currency, cfg api.AutopilotConfig, cs api.ConsensusState, fee types.Currency, currentPeriod uint64, rs api.RedundancySettings, hosts []api.HostInfo) bool {

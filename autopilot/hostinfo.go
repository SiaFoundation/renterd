...
func (c *contractor) HostInfo(ctx context.Context, hostKey types.PublicKey) (api.HostInfoResponse, error) {
	state := c.ap.State()

	if state.cfg.Contracts.Allowance.IsZero() {
		return api.HostInfoResponse{}, fmt.Errorf("can not score hosts because contracts allowance is zero")
	}
	if state.cfg.Contracts.Amount == 0 {
		return api.HostInfoResponse{}, fmt.Errorf("can not score hosts because contracts amount is zero")
	}
	if state.cfg.Contracts.Period == 0 {
		return api.HostInfoResponse{}, fmt.Errorf("can not score hosts because contract period is zero")
	}

	host, err := c.ap.bus.Host(ctx, hostKey)
	if err != nil {
		return api.HostInfoResponse{}, fmt.Errorf("failed to fetch requested host from bus: %w", err)
	}
	gs, err := c.ap.bus.GougingSettings(ctx)
	if err != nil {
		return api.HostInfoResponse{}, fmt.Errorf("failed to fetch gouging settings from bus: %w", err)
	}
	rs, err := c.ap.bus.RedundancySettings(ctx)
	if err != nil {
		return api.HostInfoResponse{}, fmt.Errorf("failed to fetch redundancy settings from bus: %w", err)
	}
	cs, err := c.ap.bus.ConsensusState(ctx)
	if err != nil {
		return api.HostInfoResponse{}, fmt.Errorf("failed to fetch consensus state from bus: %w", err)
	}
	fee, err := c.ap.bus.RecommendedFee(ctx)
	if err != nil {
		return api.HostInfoResponse{}, fmt.Errorf("failed to fetch recommended fee from bus: %w", err)
	}
	c.mu.Lock()
...
func (c *contractor) HostInfos(ctx context.Context, filterMode, usabilityMode, addressContains string, keyIn []types.PublicKey, offset, limit int) ([]api.HostInfoResponse, error) {
	...
	var hostInfos []api.HostInfoResponse
	wanted := limit
	for {
		...
		for _, host := range hosts {
			...
			if !cached {
				...
				if usabilityMode == api.UsabilityFilterModeAll {
					hostInfos = append(hostInfos, api.HostInfoResponse{
						Host: host,
					})

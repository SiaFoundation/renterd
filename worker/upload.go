...
type (
	...
	upload struct {
		...
		mu   sync.Mutex
		used map[slabID]map[types.PublicKey]struct{}
	}
	...
	sectorUpload struct {
		...
		mu          sync.Mutex
		overdriving map[types.PublicKey]struct{}
	}
...
func (mgr *uploadManager) newUpload(ctx context.Context, totalShards int, contracts []api.ContractMetadata, bh uint64, lockPriority int) (*upload, func(), error) {
	...
	return &upload{
		...
		used: make(map[slabID]map[types.PublicKey]struct{}),
	}, finishFn, nil
}
...
func (mgr *uploadManager) numUploaders(u *upload) (n int) {
	...
	for _, uploader := range mgr.uploaders {
		if _, allowed := u.allowed[uploader.hk]; allowed {
			n++
		}
	}
	return
...
func (mgr *uploadManager) refreshUploaders(contracts []api.ContractMetadata, bh uint64) {
	...
	for _, uploader := range mgr.uploaders {
		fcid, endHeight := uploader.contractInfo()
		_, renewed := c2r[fcid]
...
func (u *upload) newSlabUpload(ctx context.Context, shards [][]byte, mem *acquiredMemory) (*slabUpload, []*sectorUploadReq, chan sectorUploadResp) {
	...
	for sI, shard := range shards {
		...
		sector := &sectorUpload{
			...
			ctx:         sCtx,
			cancel:      sCancel,
			overdriving: make(map[types.PublicKey]struct{}),
		}
...
func (u *upload) canUseUploader(sID slabID, ul *uploader) bool {
	if _, allowed := u.allowed[ul.hk]; !allowed {
		return false
	}

	_, used := u.used[sID][ul.hk]
	return !used
}
...
func (u *upload) markUsed(sID slabID, hk types.PublicKey) {
	u.mu.Lock()
	defer u.mu.Unlock()

	_, exists := u.used[sID]
	if !exists {
		u.used[sID] = make(map[types.PublicKey]struct{})
	}
	u.used[sID][hk] = struct{}{}
}

func (u *upload) markUnused(sID slabID, hk types.PublicKey) {
	u.mu.Lock()
	defer u.mu.Unlock()
	delete(u.used[sID], hk)
}
...
func (u *uploader) contractInfo() (types.FileContractID, uint64) {
	u.mu.Lock()
	defer u.mu.Unlock()
	return u.fcid, u.bh
}
...
outer:
	for {
		...
		for {
			...
			start := time.Now()
			fcid, _ := u.contractInfo()
			err := rl.withRevision(req.sector.ctx, defaultRevisionFetchTimeout, fcid, u.hk, u.siamuxAddr, req.upload.lockPriority, u.blockHeight(), func(rev types.FileContractRevision) error {
...
func (u *uploader) enqueue(req *sectorUploadReq) {
	...
	// mark as used
	req.upload.markUsed(req.sID, u.hk)
...
func (s *slabUpload) launch(req *sectorUploadReq) (interrupt bool, err error) {
	...
	if req.overdrive {
		...
		req.sector.mu.Lock()
		req.sector.overdriving[req.hk] = struct{}{}
...
func (s *slabUpload) overdrive(ctx context.Context, respChan chan sectorUploadResp) (resetTimer func()) {
	...
	canOverdrive := func() bool {
		...
		remaining := uint64(len(s.shards)) - s.numUploaded
		if remaining >= s.mgr.maxOverdrive {
			return false
		}
		...
		if s.numInflight-remaining >= s.mgr.maxOverdrive {
			return false
		}
...
func (s *slabUpload) receive(resp sectorUploadResp) bool {
	...
	if req.overdrive {
		sector.mu.Lock()
		delete(sector.overdriving, req.hk)
		sector.mu.Unlock()
	}

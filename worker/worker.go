...
type Bus interface {
	...
	UpdateSlab(s object.Slab, usedContracts map[consensus.PublicKey]types.FileContractID, migratedHosts []consensus.PublicKey) error
...
func (w *worker) slabsMigrateHandler(jc jape.Context) {
	...
	originalHosts := make(map[consensus.PublicKey]struct{})
	for _, ss := range slab.Shards {
		originalHosts[ss.Host] = struct{}{}
	}
	...
	for _, ss := range slab.Shards {
		...
		delete(originalHosts, ss.Host)
	}

	migratedHosts := make([]consensus.PublicKey, 0, len(originalHosts))
	for k := range originalHosts {
		migratedHosts = append(migratedHosts, k)
	}

	err = w.bus.UpdateSlab(slab, usedContracts, migratedHosts)

...
func (c *Client) RHPPruneContract(ctx context.Context, fcid types.FileContractID) (pruned int64, err error) {
	err = c.c.WithContext(ctx).POST(fmt.Sprintf("/rhp/contract/%s/prune", fcid), nil, &pruned)
	return
}

...
type (
	...
	ObjectStore interface {
		...
		PutSlab(s object.Slab, usedContracts map[consensus.PublicKey]types.FileContractID, migratedHosts []consensus.PublicKey) error
	}
...
func (b *bus) slabHandlerPUT(jc jape.Context) {
	...
	if jc.Decode(&usr) == nil {
		jc.Check("couldn't update slab", b.os.PutSlab(usr.Slab, usr.UsedContracts, usr.MigratedHosts))
	}
}
...
func (b *bus) slabsMigrationHandlerGET(jc jape.Context) {
	var set string
	var limit int
	if jc.DecodeForm("set", &set) == nil {
		if jc.DecodeForm("limit", &limit) == nil {
			if slabs, err := b.os.SlabsForMigration(set, limit); jc.Check("couldn't fetch slabs for migration", err) == nil {
				jc.Encode(slabs)
			}
		}
	}

...
func TestUploadedSectorsCache(t *testing.T) {
	...
	if roots2 := c.sectors(types.FileContractID{2}); len(roots2) != 2 || roots2[0] != (types.Hash256{2}) || roots2[1] != (types.Hash256{3}) {
	...
	if err := c.addUploadingSector(uID1, types.FileContractID{1}, types.Hash256{1}); err != api.ErrUnknownUpload {
		t.Fatal("unexpected error", err)
	}
	if err := c.trackUpload(uID1); err != nil {
		t.Fatal("unexpected error", err)
	}
	if err := c.trackUpload(uID1); err != api.ErrUploadAlreadyExists {
		t.Fatal("unexpected error", err)
	}
}

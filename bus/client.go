...
func (c *Client) SlabsForMigration(set string, limit int) (slabs []object.Slab, err error) {
	...
	values.Set("set", set)
	values.Set("limit", fmt.Sprint(limit))
	err = c.c.GET("/slabs/migration?"+values.Encode(), &slabs)
	return
}

// UpdateSlab updates the given slab in the database.
func (c *Client) UpdateSlab(slab object.Slab, usedContracts map[consensus.PublicKey]types.FileContractID, migratedHosts []consensus.PublicKey) (err error) {
	err = c.c.PUT("/slabs", api.UpdateSlabRequest{
		Slab:          slab,
		UsedContracts: usedContracts,
		MigratedHosts: migratedHosts,
	})
	return
}

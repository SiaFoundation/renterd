...
func (c chainUpdateTx) UpdateHost(hk types.PublicKey, ha chain.HostAnnouncement, bh uint64, blockID types.BlockID, ts time.Time) error { //
	...
	// prepare statements
	insertAllowlistLinkStmt, err := c.tx.Prepare(c.ctx, "INSERT IGNORE INTO host_allowlist_entry_hosts (db_allowlist_entry_id, db_host_id) VALUES (?,?)")
	if err != nil {
		return fmt.Errorf("failed to prepare statement, %w", err)
	}
	defer insertAllowlistLinkStmt.Close()

	insertBlocklistLinkStmt, err := c.tx.Prepare(c.ctx, "INSERT IGNORE INTO host_blocklist_entry_hosts (db_blocklist_entry_id, db_host_id) VALUES (?,?)")
	if err != nil {
		return fmt.Errorf("failed to prepare statement, %w", err)
	}

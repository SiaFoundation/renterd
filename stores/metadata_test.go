...
func TestObjectBasic(t *testing.T) {
	...
	// add the object
	if err := db.UpdateObject(context.Background(), api.DefaultBucketName, t.Name(), testContractSet, want, testETag, map[types.PublicKey]types.FileContractID{
		hk1: fcid1,
		hk2: fcid2,
	}); err != nil {
		t.Fatal(err)
	...
	// add the object
	if err := db.UpdateObject(context.Background(), api.DefaultBucketName, t.Name(), testContractSet, want2, testETag, make(map[types.PublicKey]types.FileContractID)); err != nil {
		t.Fatal(err)
	}
...
func TestContractRoots(t *testing.T) {
	...
	// add the object.
	if err := cs.UpdateObject(context.Background(), api.DefaultBucketName, t.Name(), testContractSet, obj, testETag, map[types.PublicKey]types.FileContractID{hks[0]: fcids[0]}); err != nil {
		t.Fatal(err)
	}
...
func TestRenewedContract(t *testing.T) {
	...
	// add the object.
	if err := cs.UpdateObject(context.Background(), api.DefaultBucketName, "foo", testContractSet, obj, testETag, map[types.PublicKey]types.FileContractID{
		hk:  fcid1,
		hk2: fcid2,
	}); err != nil {
		t.Fatal(err)
...
func TestSQLMetadataStore(t *testing.T) {
	...
	ctx := context.Background()
	objID := "key1"
	if err := db.UpdateObject(ctx, api.DefaultBucketName, objID, testContractSet, obj1, testETag, usedHosts); err != nil {
		t.Fatal(err)
	}
...
func TestObjectHealth(t *testing.T) {
	...
	if err := db.UpdateObject(context.Background(), api.DefaultBucketName, "/foo", testContractSet, add, testETag, map[types.PublicKey]types.FileContractID{
		hks[0]: fcids[0],
		hks[1]: fcids[1],
		hks[2]: fcids[2],
		hks[3]: fcids[3],
	...
	if err := db.UpdateObject(context.Background(), api.DefaultBucketName, "/bar", testContractSet, add, testETag, nil); err != nil {
		t.Fatal(err)
	}
...
func TestObjectEntries(t *testing.T) {
	...
	for _, o := range objects {
		...
		err := os.UpdateObject(ctx, api.DefaultBucketName, o.path, testContractSet, obj, testETag, ucs)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestSearchObjects(t *testing.T) {
	...
	for _, o := range objects {
		...
		obj.Slabs = obj.Slabs[:1]
		obj.Slabs[0].Length = uint32(o.size)
		if err := os.UpdateObject(ctx, api.DefaultBucketName, o.path, testContractSet, obj, testETag, ucs); err != nil {
			t.Fatal(err)
		}
	}
	tests := []struct {
...
func TestUnhealthySlabs(t *testing.T) {
	...
	ctx := context.Background()
	if err := db.UpdateObject(ctx, api.DefaultBucketName, "foo", testContractSet, obj, testETag, map[types.PublicKey]types.FileContractID{
		hk1: fcid1,
		hk2: fcid2,
		hk3: fcid3,
		hk4: fcid4,
...
func TestUnhealthySlabsNegHealth(t *testing.T) {
	...
	// add the object
	ctx := context.Background()
	if err := db.UpdateObject(ctx, api.DefaultBucketName, "foo", testContractSet, obj, testETag, map[types.PublicKey]types.FileContractID{hk1: fcid1}); err != nil {
		t.Fatal(err)
	}
...
func TestContractSectors(t *testing.T) {
	...
	ctx := context.Background()
	if err := db.UpdateObject(ctx, api.DefaultBucketName, "foo", testContractSet, obj, testETag, usedContracts); err != nil {
		t.Fatal(err)
	}
...
func TestRenameObjects(t *testing.T) {
	...
	for _, path := range objects {
		...
		if err := cs.UpdateObject(ctx, api.DefaultBucketName, path, testContractSet, obj, testETag, ucs); err != nil {
			t.Fatal(err)
		}
	}
...
func TestObjectsStats(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		err := cs.UpdateObject(context.Background(), api.DefaultBucketName, key, testContractSet, obj, testETag, contracts)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestPartialSlab(t *testing.T) {
	...
	err = db.UpdateObject(context.Background(), api.DefaultBucketName, "key", testContractSet, obj, testETag, usedContracts)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = db.UpdateObject(context.Background(), api.DefaultBucketName, "key2", testContractSet, obj2, testETag, usedContracts)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = db.UpdateObject(context.Background(), api.DefaultBucketName, "key3", testContractSet, obj3, testETag, usedContracts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestContractSizes(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		}, testETag, map[types.PublicKey]types.FileContractID{
			hks[i]: fcids[i],
		}); err != nil {
			t.Fatal(err)
		}
...
func TestObjectsBySlabKey(t *testing.T) {
	...
	for _, name := range []string{"obj1", "obj2", "obj3"} {
		obj.Slabs[0].Length++
		err = db.UpdateObject(context.Background(), api.DefaultBucketName, name, testContractSet, obj, testETag, usedContracts)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestBucketObjects(t *testing.T) {
	...
	obj, ucs := newTestObject(1)
	err = os.UpdateObject(context.Background(), "unknown-bucket", "foo", testContractSet, obj, testETag, ucs)
	...
	for _, o := range objects {
		...
		obj.Slabs = obj.Slabs[:1]
		obj.Slabs[0].Length = uint32(o.size)
		err := os.UpdateObject(ctx, o.bucket, o.path, testContractSet, obj, testETag, ucs)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestCopyObject(t *testing.T) {
	...
	// Create one object.
	obj, ucs := newTestObject(1)
	err = os.UpdateObject(ctx, "src", "/foo", testContractSet, obj, testETag, ucs)
	if err != nil {
		t.Fatal(err)
	}

...
func TestBusRecordedMetrics(t *testing.T) {
	...
	for _, m := range csMetrics {
		...
		} else if !m.Timestamp.Std().Equal(startTime) {
			t.Fatal("expected time to match start time", m.Timestamp.Std(), startTime)
		}
	}
	...
	} else if m := cMetrics[0]; !startTime.Equal(m.Timestamp.Std()) {
		t.Fatal("expected time to match start time", m.Timestamp.Std(), startTime)
	} else if m.ContractID == (types.FileContractID{}) {
		t.Fatal("expected non-zero FCID")

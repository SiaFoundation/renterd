...
func NewBus(cfg BusConfig, dir string, walletKey types.PrivateKey) (http.Handler, func() error, error) {
	...
	b, busCleanup, err := bus.New(syncer{g, tp}, chainManager{cs: cs}, txpool{tp}, w, sqlStore, sqlStore, sqlStore, sqlStore, sqlStore, cfg.GougingSettings, cfg.RedundancySettings, cfg.InteractionFlushInterval)
	if err != nil {
		return nil, nil, err
	}

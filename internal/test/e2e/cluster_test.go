...
func TestDownloadAllHosts(t *testing.T) {
	...
	for i, slab := range obj.Slabs {
		for j := range slab.Shards {
			frand.Read(obj.Slabs[i].Shards[j].LatestHost[:])
		}
	}
	tt.OK(cluster.Bus.AddObject(context.Background(), testBucket, path, test.ContractSet, *obj.Object, api.AddObjectOptions{}))

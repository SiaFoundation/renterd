...
func (ss *SQLStore) PutSlab(s object.Slab, usedContracts map[consensus.PublicKey]types.FileContractID, migratedHosts []consensus.PublicKey) error {
	...
	// extract host keys
	hostkeys := make([]consensus.PublicKey, 0, len(usedContracts))
	for key := range usedContracts {
		hostkeys = append(hostkeys, key)
	}

	// extract file contract ids
	fcids := make([]types.FileContractID, 0, len(usedContracts))
	...
	if err := ss.db.
		Model(&dbHost{}).
		Where("public_key IN (?)", gobEncode(hostkeys)).
		Find(&dbHosts).
		Error; err != nil {
		return err
	}
	...
	for _, contract := range dbContracts {
		contracts[contract.FCID] = &contract
	}
	...
	return ss.db.Transaction(func(tx *gorm.DB) (err error) {
		...
		shards := make(map[uint]dbShard)
		for _, shard := range slab.Shards {
			shards[shard.DBSectorID] = shard
		}
		...
		for _, shard := range s.Shards {
			...
			if err := tx.
				...
				Append(contracts[usedContracts[shard.Host]]); err != nil {
				return err
			}
			if err := tx.
				Model(&sector).
				Association("Hosts").
				Append(hosts[shard.Host]); err != nil {
				return err
			}

			// TODO: use migrated hosts to update the associations (?)
		}
		return nil
	})
}
...
func (s *SQLStore) SlabsForMigration(set string, limit int) ([]object.Slab, error) {
	...
	if err := s.db.
		...
		FindInBatches(&dbBatch, 10000, func(tx *gorm.DB, batch int) error {
			fmt.Println("found batch", len(dbBatch))

...
func TestUploaderStopped(t *testing.T) {
	...
	ul := New(context.Background(), cl, cs, hm, api.HostInfo{}, md.ID, md.WindowEnd, 180, zap.NewNop().Sugar())
...
func TestRefreshUploader(t *testing.T) {
	...
	// create uploader
	ul := New(context.Background(), cl, cs, hm, hi, c.ID, c.WindowEnd, 180, zap.NewNop().Sugar())
...
func TestUploaderQueue(t *testing.T) {
	...
	ul := New(context.Background(), cl, cs, hm, api.HostInfo{PublicKey: hk}, fcid, 0, 180, zap.NewNop().Sugar())

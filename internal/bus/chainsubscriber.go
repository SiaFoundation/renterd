...
func (s *chainSubscriber) revertV1ContractUpdate(tx sql.ChainUpdateTx, fce types.FileContractElement, rev *types.FileContractElement, resolved, valid bool) error {
	fcid := fce.ID
	if rev != nil {
		fcid = rev.ID
	}
	...
	if rev == nil && fce.FileContract.RevisionNumber == 0 {
